...
app = app.replace(
  /bg-\[rgba\(254,246,222,0\.7\)\] backdrop-blur-sm border-b border-\[#FFE0CE\]\/50 shadow-sm/g,
  'bg-[#FEF6DE]/70 backdrop-blur-md border-b border-[#FFE0CE]/50 shadow-sm'
);
...
leaderboard = leaderboard.replace(
  /bg-\[rgba\(255,253,242,0\.75\)\] hover:bg-\[rgba\(255,253,242,0\.9\)\]/g,
  'bg-[#FFFDF2]/60 hover:bg-[#FFFDF2]/80'
);
...
css = css.replace(/\.glass-card \{[\s\S]*?\}/, `.glass-card {
  ...
  border: 1px solid rgba(255, 224, 206, 0.5);
